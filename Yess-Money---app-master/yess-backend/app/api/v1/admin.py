...
MOCK_REFERRALS = [
    Referral(id=i, referrer_id=(i % 10) + 1, referred_id=(i % 20) + 11, status="completed" if i % 2 == 0 else "pending", reward=100.0 * i)
    for i in range(1, 51)
]
...
async def get_referrals():
    ...
    return {"data": [r.dict() for r in MOCK_REFERRALS]}

@router.get("/referrals/stats")
async def get_referrals_stats():
    """Получить статистику рефералов"""
    total = len(MOCK_REFERRALS)
    completed = len([r for r in MOCK_REFERRALS if r.status == "completed"])
    pending = len([r for r in MOCK_REFERRALS if r.status == "pending"])
    total_reward = sum([r.reward for r in MOCK_REFERRALS if r.status == "completed"])
...
# ========== Promotions PUT endpoint ==========

@router.put("/promotions/{promotion_id}")
async def update_promotion(promotion_id: int, promotion_data: Promotion):
    """Обновить акцию"""
    promotion = next((p for p in MOCK_PROMOTIONS if p.id == promotion_id), None)
    if not promotion:
        raise HTTPException(status_code=404, detail="Promotion not found")
...
async def delete_promotion(promotion_id: int):
    ...
    MOCK_PROMOTIONS = [p for p in MOCK_PROMOTIONS if p.id != promotion_id]
