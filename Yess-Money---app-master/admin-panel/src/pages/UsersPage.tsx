...
export const UsersPage = () => {
  ...
  const { data: usersData, isLoading, refetch } = useQuery({
    queryKey: [...queryKeys.users, page, pageSize, debouncedSearchText],
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      totalBalance: filteredUsers.reduce((sum, u) => sum + (u.balance || 0), 0),
            ...
            <Input
              ...
              suffix={
                searchText && searchText !== debouncedSearchText ? (
                  <span style={{ color: '#689071', fontSize: 12 }}>{t('common.loading', 'Загрузка...')}</span>
                ) : null
              }
            />
            {debouncedSearchText && (
              <div style={{ marginTop: 8, fontSize: 12, color: '#689071' }}>
                {t('users.found', 'Найдено результатов')}: {total}
              </div>
            )}
          </Col>
          <Col xs={24} sm={12} md={6}>
              ...
              >
                {users.map(user => (
                  <Select.Option key={user.id} value={user.id}>
                    {user.name} ({user.balance.toLocaleString()} Y)
                  </Select.Option>
                ))}
              </Select>
            </Form.Item>
