    ...
    debounceClick(async () => {
      ...
      try {
        ...
        if (response && response.access_token) {
          ...
          const userData = response.partner || response.user || {
          ...
          setErrorMessage(t('login.invalidCredentials', 'Неверные учетные данные'));
          message.error(t('login.invalidCredentials', 'Неверные учетные данные'));
        }
      } catch (error: any) {
        ...
        message.error(toastMessage || t('login.error', 'Ошибка при входе'));
      } finally {
        setLoading(false);
      }
    });
  ...
  const onFinishFailed = (errorInfo: any) => {
    ...
    if (firstError) {
      ...
      if (fieldName === 'username') {
        errorText = t('login.fieldError', '⚠️ Ошибка в поле "Имя пользователя": {error}', { error: errorMsg });
      } else if (fieldName === 'password') {
        errorText = t('login.fieldError', '⚠️ Ошибка в поле "Пароль": {error}', { error: errorMsg });
      } else {
      ...
      <div className="login-right">
        <div className="login-form-container">
          ...
          <div className="login-header">
            <h2>{t('login.title', 'Partner Panel Login')}</h2>
          ...
          <div className="login-demo">
            ...
            <div className="login-demo-field">{t('login.demoUsername', 'Username: partner')}</div>
            <div className="login-demo-field">{t('login.demoPassword', 'Password: partner')}</div>
          </div>
        </div>
