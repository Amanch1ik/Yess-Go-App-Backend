...
// Интерцептор для обработки ошибок с локализованными сообщениями
apiClient.interceptors.response.use(
  ...
  (error: AxiosError) => {
    ...
    if (error.response) {
      ...
      switch (status) {
        case 401:
          // Токен истек или невалиден - автоматический logout
          localStorage.removeItem('partner_token');
          localStorage.removeItem('partner_user');
          if (window.location.pathname !== '/login') {
            // Используем navigate вместо window.location для плавного перехода
            setTimeout(() => {
              window.location.href = '/login';
            }, 100);
          }
          // Не логируем в консоль для production
